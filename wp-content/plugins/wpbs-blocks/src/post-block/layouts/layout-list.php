<?php
/**
 * List layout for Post Block
 */

$wrapper_class = 'post-block-wrapper post-layout-list';
?>
<section class="<?php echo esc_attr($wrapper_class); ?>">
    <div class="posts-list">
        <?php while ($posts_query->have_posts()) : $posts_query->the_post(); ?>
            <article id="post-<?php the_ID(); ?>" class="blog-card layout-list">
                <?php if ($display_featured_image && has_post_thumbnail()) : ?>
                    <div class="post-image">
                        <img src="<?php echo esc_url(get_the_post_thumbnail_url(get_the_ID(), 'medium')); ?>" alt="<?php echo esc_attr(get_the_title()); ?>">
                    </div>
                <?php endif; ?>

                <div class="blog-content">
                    <div class="post-title"><a href="<?php echo esc_url(get_permalink()); ?>"><?php the_title(); ?></a></div>
                    <?php if ($display_excerpt) : ?>
                        <div class="blog-desc"><?php if (has_excerpt()) { echo wp_trim_words(get_the_excerpt(), 18, '...'); } else { echo wp_trim_words(get_the_content(), 18, '...'); } ?></div>
                    <?php endif; ?>
                </div>
            </article>
        <?php endwhile; ?>
    </div>
</section>
